...
def dedequantize_tensor(q_x, scale, zero_point):
...
class Fakequant(nn.Module):
    ...
    def forward(self, x):
        x = self.qparam.quantize_tensor(x)
        x = self.qparam.dequantize_tensor(x)
        return x
...
class QParam(nn.Module):
    ...
    def update(self, tensor):
        ...
        if self.signed:
            self.max.clamp_(min=abs(self.min.data))
        else:
            self.max.clamp_(min=0)
    ...
    def dedequantize_tensor(self, q_x):
        return dedequantize_tensor(q_x, self.scale, self.zero_point)
...
class QLinear(QModule):
    ...
    def __init__(self, fc_module, qi=True, qo=True, num_bits=8, signed=True):
...
class QAvgpool(QModule):

    def __init__(self, qo=False, num_bits=None):
        super(QAvgpool, self).__init__(qo=qo, num_bits=num_bits)
...
class QConvBNReLU(QModule):
    ...
    def __init__(self, conv_module, bn_module, qi=True, qo=True, num_bits=8, bias_max = 0.14, signed=False):
        super(QConvBNReLU, self).__init__(qi=qi, qo=qo, num_bits=num_bits)
        self.num_bits = num_bits
        self.conv_module = conv_module
        self.bn_module = bn_module
        self.bias_max = bias_max # 控制relu阈值
        self.qw = QParam(num_bits=num_bits, signed=signed)
        self.qb = QParam(num_bits=32)
        # self.offset = nn.Parameter(torch.rand(1, self.conv_module.out_channels, 1, 1)*0.01)
        self.register_buffer('M', torch.tensor([], requires_grad=False))  # 将M注册为buffer

    def fold_bn(self, mean, std):
        if self.bn_module.affine:
            gamma_ = self.bn_module.weight / std
            # self.conv_module.weight = self.conv_module.weight.cuda()
    ...
    def forward(self, x):
        ...
        bias.data.clamp_(max=self.bias_max) # add
    ...
    def freeze(self, qi=None, qo=None):
        ...
        qb = quantize_tensor(bias, scale=self.qi.scale * self.qw.scale,
                                                     zero_point=0, num_bits=32, signed=True)
        qb = qb.cpu().detach()
        self.conv_module.bias = nn.Parameter(qb, requires_grad=False)
...
class QConvBN(QModule):
    ...
    def __init__(self, conv_module, bn_module, qi=True, qo=True, num_bits=8, signed=False):
...
class QIdentityReLU(nn.Module):
    def __init__(self, conv_module, qi=True, qo=True, num_bits=8):
        super(QIdentityReLU, self).__init__()
        self.num_bits = num_bits
        if qi:
            self.qi_x = QParam(num_bits=num_bits)
            self.qi_iden = QParam(num_bits=num_bits)
        if qo:
            self.qo = QParam(num_bits=num_bits)
        
        # self.offset = nn.Parameter(torch.rand(1, conv_module.out_channels, 1, 1)*0.01)
        self.register_buffer('M1', torch.tensor([], requires_grad=False))  # 将M注册为buffer
        self.register_buffer('M2', torch.tensor([], requires_grad=False))  # 将M注册为buffer
    
    def forward(self, x, identity):
        if hasattr(self, 'qi'):
            self.qi_x.update(x)
            self.qi_iden.update(x)
            x = FakeQuantize().apply(x, self.qi_x)
            identity = FakeQuantize().apply(identity, self.qi_iden)
        
        x += identity

        # self.offset.data.clamp_(min=0.01)
        # x = x - self.offset
    ...
    def freeze(self, qi1=None, qi2=None, qo=None):
        ...
        self.M2.data = (self.qi2.scale / self.qi1.scale).data
    
    def quantize_inference(self, x, identity):
        x = x - self.qi1.zero_point
        iden = self.M2 * (identity - self.qi2.zero_point)
        x = self.M1 * (x + iden)
        x.round_() 
        x = x + self.qo.zero_point        
        x.clamp_(0., 2.**self.num_bits-1.).round_()
    ...
    def forward_fp_output(self, x, identity):
        if hasattr(self, 'qi'):
            self.qi.update(x)
            x = FakeQuantize().apply(x, self.qi)
