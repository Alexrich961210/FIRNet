...
def get_imagenet_iter_torch(type, image_dir, batch_size, num_threads, device_id, num_gpus=1, crop=224, val_size=256,
                            world_size=1, local_rank=0, chunk=0, chunk_size=1, count=False):
    if type == 'train':
        ...
        if count:
            sequential = SubsetRandomSampler(range(chunk*chunk_size, (chunk+1)*chunk_size))
            dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_threads, sampler=sequential,
                                                 pin_memory=False)
        else:
            dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_threads,
                                                 pin_memory=False)
        
        # subset_indices = range(0, 2, 1)
